...
(async () => {
  ...
  try {
    sma50Urls = [
      "https://www.screener.in/screens/2740099/stocks-above-200-day-sma/",
      "https://www.screener.in/screens/2740103/stocks-below-200-day-sma/",
    ];

    numbers = [];

    for (let i = 0; i < 2; i++) {
      await page.goto(sma50Urls[i], {
      ...
      const nofStocks = await page.evaluate(() => {
        return parseInt(
          document
            .querySelector(".sub[data-page-info]")
            ?.textContent.trim()
            .split(" ")[0]
        );
      });
    ...
    fs.writeFileSync(
      path.join(__dirname, "..", "data", "sma200.json"),
      JSON.stringify(data, null, 2)
    );

    console.log("Scraped Above & Below 200 SMA:", data);
  } catch (err) {
